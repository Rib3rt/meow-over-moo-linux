...
%PY_CMD% "%SCRIPT_DIR%\scripts\build_native_linux_package.py" --source-project "%SCRIPT_DIR%" --linux-runtime-dir "%APPIMAGE_DIR%" --output-parent "%SCRIPT_DIR%\.." --strip-steam-appid
if errorlevel 1 goto :build_failed

echo.
echo Linux native release package build completed.
...
:build_failed
echo [ERROR] Linux release package build failed.
exit /b 1
