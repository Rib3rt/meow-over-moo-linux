...
echo [2/2] Building final Windows test package and zip...
for /f "delims=" %%I in ('cmd /c %PYTHON_CMD% "%PROJECT_ROOT%\scripts\build_fused_windows_package.py" --source-project "%PROJECT_ROOT%" --love-runtime-dir "%LOVE_RUNTIME_DIR%" --output-parent "%OUTPUT_PARENT%" --openal-override-dir "%OPENAL_OVERRIDE_DIR%" --zip-package') do (
...
if not defined PACKAGE_DIR (
    echo Failed to build the final Windows test package.
    exit /b 1
)

echo.
echo Final test package folder:
echo %PACKAGE_DIR%
